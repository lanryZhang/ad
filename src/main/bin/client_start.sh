...
# ?m
     ...
     if [ -z "$7" ]
then
        MEM_OPTIONS="-Xms4g -Xmx4g -XX:NewRatio=4 -XX:SurvivorRatio=8"
else
        MEM_OPTIONS=$7
fi
...
java -server -Dcom.sun.management.jmxremote  -Dsun.lang.ClassLoader.allowArraySyntax=true $MEM_OPTIONS -XX:TargetSurvivorRatio=90 -XX:MaxTenuringThreshold=4 -XX:+PrintTenuringDistribution -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -verbose:gc -cp $CP -D$PROGRAM_HOME_NAME=$PROGRAM_HOME $SERVER_CLASS client $5 >$LOG_FILE 2>&1 &
echo $! > $PID_FILE
